...
For this repository, `__PROJECT_MANAGER__` is one developer profile, but it is not the default for everyone.
...
- Do not infer a developer profile from git config, file ownership, previous sessions, branch names, or the existence of `.agents/developers/__PROJECT_MANAGER__/`.
- Before a profile is confirmed, only read `AGENTS.md` and `.agents/common/`.
- A valid profile confirmation should name the profile explicitly, for example: `Use the __PROJECT_MANAGER__ profile.`
- After confirmation and before reading or writing any personal directory, state the active profile and write boundary, for example: `Current developer profile: __PROJECT_MANAGER__. Only read and write .agents/developers/__PROJECT_MANAGER__/.`
...
```text
Current developer profile: __PROJECT_MANAGER__. Only read and write .agents/developers/__PROJECT_MANAGER__/.

Context recovery complete:
- Read `AGENTS.md`
- Read `.agents/common/rules.md`
- Read `.agents/common/context.md`
- Read `.agents/developers/__PROJECT_MANAGER__/AGENTS.md`
- Read `.agents/developers/__PROJECT_MANAGER__/progress.md`
- Read `.agents/developers/__PROJECT_MANAGER__/bugs.md`
...
- Ask the developer to input their nickname or profile name explicitly, for example `__PROJECT_MANAGER__`.
...
- Only `__PROJECT_MANAGER__` may directly approve and merge changes to `.agents/common/rules.md` or `.agents/common/context.md`.
...
- Until approved by `__PROJECT_MANAGER__`, proposed common changes are not active rules.
...
4. For stable multi-developer facts, submit a common proposal or ask `__PROJECT_MANAGER__` to approve the common change.
